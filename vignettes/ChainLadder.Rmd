...
Function `triangle` builds a triangle matrix from the vectors of known data provided in `...`. Normally, each of these vectors should be one shorter than the preceding one. The length of the first vector dictates the number of development periods or origin periods (respectively when bycol is `FALSE` or `TRUE`).
...
We can specify the *labels* for the origin periods:
```{r}
...
and also specify the labels for development periods:
...
As a special case, the function will build an $n \times n$ triangle from a single vector of $n(n + 1)/2$ data points.
...
Starting from data stored in table format (for example `data.frame`), we can use the `as.triangle` function to convert data into triangle format:

```{r}
origin = c(2015, 2015, 2015, 2016, 2016, 2017)
dev = c(1, 2, 3, 1, 2, 1)
value = c(100, 150, 175, 110, 168, 115)
d = data.frame(origin,dev,value)
d
tri = as.triangle(d)
tri
```
...
For a data set of class triangle the `ChainLadder` package provides default plotting
methods to give a graphical overview of the data:

```{r}
# we load an example dataset provided with the package
tri = ChainLadder::RAA
plot(tri)
```
