...
* Changes
    ...
    + Simplified model extraction from lm
    + approximately unity ata
    + New unit tests
* Bug Fixes
...
to numeric.
Curiously, sometimes that "requirement" did not seem to cause a problem.
For example, the following sets the rownames of GenIns to the beginning Date
of the accident year, and the default call to plot displays the triangle without error
...
However, a plot with the lattice=TRUE option would blow up.
Now it displays with the pretty headings.
...
As it turns out, 
this need for "origin" values that are 
not necessarily convertible to numeric
arises frequently.

Suppose you have a table of claim detail at various evaluation dates.
Invariable, such a table will have a date field holding the date of loss.
...
In this example we take the GenIns data in long format and
generate 50 claims per accident period.
We assign each claim a random date within the year.
The value given is a random perturbation of one-fiftieth of
the value in GenInsLong.
...
has more informative row labels.
...
Here is an example, also found in ?glmReserve.
...
Test for essentially-unity link ratio to within a reasonable tolerance.
