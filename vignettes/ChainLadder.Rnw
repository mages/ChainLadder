...
\end{equation}
where $A_k$ is a column of intercepts and $B_k$ is the a development matrix. Assumptions for this model are:
\begin{align}
...
\end{align}
In the above, $D$ is the diagonal opterator, and $\delta$ is a known positive value that controls how the variance depends on the mean (as weights).

The simplest case where $A_k=0$ and $B_k$ are diagonal is a naive generalizatio of the chain ladder, often referred to as the multivariate chain ladder (see Pr\"{o}hl and Schmidt 2005). 
The following shows an application of this case. The following is a data set used in Merz and W\"{u}thrich (2008):
<<>>=
data(liab)
# this is a list of two triangles 
length(liab)
dim(liab[[1]])
@
If we specify \texttt{fit.method = "OLS"}, the ordinary lease squares will be used in estimating the development factors. As a result, the multivariate model is equivalent to running two Mack chain ladders separately. 
<<>>=
fit1 <- MultiChainLadder(liab, fit.method = "OLS")
(s1 <- summary(fit1))
@
By default, the \texttt{summary} produces reserve statistics for all individual triangles, as well as for the portfolio that is assumed to be the sum of the two triangles. This behavior can be changed by supplying the \texttt{portfolio} argument. See the documentation for details. 

We can verify if this is indeed the same as the univariate Mack chain ladder:
<<>>=
MackChainLadder(liab[[1]], est.sigma = "Mack")
@

The \texttt{model.summary} slot in the summary stores the fitted parameters in the regression fit for each development period: 
<<>>=
names(s1)
head(s1$model.summary)
@

To allow correlations to be incorporated, we employ the seemingly unrelated regressions that simultaneously model the two triangles. We now specify \texttt{fit.method = "SUR"}:
<<>>=
fit2 <- MultiChainLadder(liab, fit.method = "SUR")
(s2 <- summary(fit2))
@

We see that the portfolio prediction error is inflated to $500,607$ from $457,278$ due to the positive correlations between the two triangles. The estimated correlation for each development period can be retrieved through the \texttt{residCor} function: 
<<>>=
unlist(residCor(fit2))
@
Similarly, most methods that work for linear models such as \texttt{coef}, \texttt{fitted}, \texttt{resid} and so on will also work. For example, we can retrieve the estimated development factors for the first two periods as 
<<>>=
coef(fit2)[1:2]
@

The plot function can also be used to generate various summary and diagonostic figures:

\begin{figure}[h]
\centering
<<fig = TRUE, label = MultiPlot, width = 6, height = 11>>=
parold <- par(mfrow = c(4, 2), mar = c(4, 4, 2, 1), 
    mgp = c(1.3, 0.3, 0), tck = -0.02)
plot(fit2, which.triangle = 1:2, which.plot = c(1, 3, 4))
par(parold)
@
\end{figure}
We use the \texttt{which.triangle} to suppress the plot for the portfolios, and use the \texttt{which.plot} to select the desired types of plots. See the documentation for possible values for these two arguments. 
...
fs1 <- MultiChainLadder(liab1, extrap = FALSE, int = 1:9, model = "GMCL")
fs2 <- MultiChainLadder(liab2, fit.method = "OLS")
fs <- Join2Fits(fs1, fs2)
ffs <- predict(fs)               
mse.mack  <-  Mse(fs, ffs, "Mack")
fit3 <- JoinFitMse(fs, mse.mack)
plot(fit3, which.triangle = 1:2, which.plot = 3)
@ 
