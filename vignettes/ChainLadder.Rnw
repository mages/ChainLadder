...
The resulting plots are shown in Figure \ref{fig:multi}. We use \texttt{which.triangle} to suppress the plot for the portfolio, and use the \texttt{which.plot} to select the desired types of plots. See the documentation for possible values of these two arguments. 
...
\begin{figure}[!h]
\centering
<<fig = TRUE, label = ReservePlot,  width = 6, height = 4>>=
library(ggplot2)
library(reshape2)
prm <- melt(pr)
