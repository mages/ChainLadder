...
\hypersetup{
  ...
  pdfauthor={Markus Gesmann, Dan Murphy, Mario W\"{u}thrich, Wayne Zhang, Alessandro Carrato},%
}
...
\subsubsection{Copying and pasting from MS Excel}
...
copy lists structures from R to Excel easily. 
...
\subsubsection{Munich chain-ladder}
 ...
 the standard error around those~\cite{Quarg2004}.
...
\subsubsection{Joint modelling of the paid and incurred losses}
...
The stochastic claims reserving  methods considered above predict the lower (unknown)
triangle and assess the uncertainty of this prediction. For instance, Mack's
uncertainty formula quantifies the total prediction uncertainty of the chain-ladder predictor over the entire run-off of the outstanding claims. Modern solvency considerations, such as Solvency II, require a second view of claims reserving uncertainty.
This second view is a short-term view because it requires assessment of
the one-year change of the claims prediction when one updates the available information
at the end of the accounting year. At time $n$ we have information
\begin{equation*}
{\cal D}_{n} 
= \left\{C_{i,k};~{i+k \le n+1} \right\},
\end{equation*}
and one year later at time $n+1$ we have information
\begin{equation*}
{\cal D}_{n+1} 
= \left\{C_{i,k};~{i+k \le n+2} \right\}.
\end{equation*}
This motivates the following two predictors for the ultimate claim $C_{i,K}$
at times $n$ and $n+1$, respectively,
\begin{equation*}
\widehat{C}^{(n)}_{i,K}= E[C_{i,K}|{\cal D}_n]
\qquad \text{ and } \qquad 
\widehat{C}^{(n+1)}_{i,K}= E[C_{i,K}|{\cal D}_{n+1}].
\end{equation*}
The one year claims development result (CDR), see Merz-W\"uthrich~\cite{Merz;Wuthrich:2008,
Merz;Wuthrich:2014}, considers the change in this one year update
\begin{equation*}
{\rm CDR}_{i,n+1}
=\widehat{C}^{(n)}_{i,K}-\widehat{C}^{(n+1)}_{i,K}.
\end{equation*}
The tower property of conditional expectation implies that the
CDR is on average 0, that is, $E[{\rm CDR}_{i,n+1}|{\cal D}_n]=0$
and the Merz-W\"uthrich formula \cite{Merz;Wuthrich:2008, Merz;Wuthrich:2014} assesses
the uncertainty of this prediction measured
by the following conditional mean square error of prediction (MSEP)
\begin{equation*}
{\rm msep}_{{\rm CDR}_{i,n+1}|{\cal D}_n}(0) = \mathbb{E} \left[\left.\left({\rm CDR}_{i,n+1}-0\right)^2
\right|{\cal D}_n \right].
\end{equation*}
...
can be estimate via the \texttt{CDR} function for objects of
...
round(cdrM, 1)
@
Or, using the \texttt{BootChainLadder} function:
<<>>=
B <- BootChainLadder(MW2014)
cdrB <- CDR(B)
round(cdrB, 1)
@

The \texttt{tweedieReserve} function offers also the option to estimate the
one year CDR, by setting the argument \texttt{rereserving=TRUE}. The main advantage
of using this function is that the user can test different regression structures, with the
one year CDR derived coherently.

\section{Model Validation}
Model validation is one of the key activities when an insurance company goes through the 
Internal Model Approval Process with the regulator. Next we will show you some examples 
on how you can use the arguments of the \texttt{tweedieReserve} function to
help validating your stochastic reserving model. 
The argument \texttt{design.type} allows you to test different regression structures. 
The classic ODP model uses the following structure: 
\begin{equation*}
Y \backsim as.factor(OY) + as.factor(DY),
\end{equation*}
(i.e. \texttt{design.type=c(1,1,0)}). This allows, together with the log link, 
to achieve the same results of the (Volume All) Chain Ladder model, 
thus the same model implied assumptions. 
A common model shortcome is when the residuals plotted by Calendar Year start 
to show a pattern, which the Chain Ladder isn't capable to model. 
In order to overcome this, the user could be then interested to change the 
regression structure in order to try to strip out these patterns. 
...
which returns the same results of the Arithmetic Separation Method 
modelling explicitly inflation parameters between consequent CY.
...
the variance VAR(Y) relates to the mean E(Y) by the following law:
\begin{equation*}
VAR[Y] = a \cdot E[Y]^p,
\end{equation*}
...
%   progressBar=FALSE)
% @
As it is possible to see in this example, the MLE of p seems to be 
...
as anyway it seems to be near the MLE.  
