...
for(i in 1:5){
    cum.cal.trend[row(cal.trend)+col(cal.trend)==i+1] <- 0.1*i
}
cum.cal.trend[row(cal.trend)+col(cal.trend)==7] <- 0.1*5+0.3
for(i in 7:14){
    cum.cal.trend[row(cal.trend)+col(cal.trend)==i+1] <- 0.1*5+0.3+0.15*(i-6)
}
...
model <- lm(paid.noise ~  dev + I(cal-1977), data=y)
## Table 3.1 page 274
summary(model)
op <- par(mfrow=c(2,2))
plot(model)
