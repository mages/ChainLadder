...
\begin{abstract}
  ...
  The \pkg{ChainLadder} package~\citep{chainladder} was developed out of 
  presentation the author gave on stochastic reserving at the Institute of Actuaries 
  in 2007 and 2008. 
  Currently the Mack-, Munich- and Bootstrap-chain-ladder methods are implemented. 
  Further the package provides an example spreadsheet, which shows how to use the
  \pkg{ChainLadder} functions within Excel using the RExcel Add-in~\citep{rexcel}. 
...
upfront received premium. Unlike other industries insurers don't know the 
production cost of their product. The estimated future claim payments have to be held 
...
e.g. motor, marine, property, etc. 
Policy information are than aggregated by class of business to view at its historical claims development over time for each year of origin. This cross-tab view of claims data looks in most cases like a triangle filled in in the top left area. The aim is to forecast future claims developments, which would fill the bottom right of the matrix, see Figure~\ref{triangle}.
\begin{figure}[h]
  \begin{center}
\includegraphics[width=0.4\textwidth]{Triangles}
    \caption{Schematic example of claims triangles}\label{triangle}
  \end{center}
\end{figure}
